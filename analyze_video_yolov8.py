...
import os
weight_path = os.path.expanduser('~/.deepface/weights/yolov8n-face.pt')
yolo_model = YOLO(weight_path) # Ultralytics face model
...
for f_idx in frames_to_check:
    ...
    for i, box in enumerate(boxes):
        ...
        perfect_face = get_raw_face(frame_rgb, box)
        ...
        print(f"Face {i+1}: Size {int(w)}x{int(h)}, Similarity to Messi: {sim:.4f}")
