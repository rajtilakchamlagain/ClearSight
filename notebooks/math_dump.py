...
def detect_faces_gpu(img_rgb):
    ...
    for i in range(len(boxes)):
        if probs[i] > 0.90:
            ...
            x1, y1, x2, y2 = box
            w, h = x2 - x1, y2 - y1
            faces.append({'box': [x1, y1, w, h], 'confidence': probs[i], 'keypoints': landmarks[i]})
    return faces
...
anchor_paths = ['../../data/darshana_face.jpeg', '../../data/darshana_face2.jpeg', '../../data/darshana_face3.jpeg']
anchor_embeddings = []
...
while cap.isOpened():
    ...
    if frame_count % 10 != 0 and frame_count < 1000:
        continue # skip some frames to be fast, but we want to see what happens around 1000
    ...
    for i, face in enumerate(faces):
        ...
        output_lines.append(f"Frame {frame_count} Face {i} dist: {distance:.3f}")

cap.release()
with open('math_dump.txt', 'w') as f:
    f.write("\n".join(output_lines))
print("DONE")
